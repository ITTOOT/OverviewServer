...
using OverviewServer.Models.User;
using System.Runtime.InteropServices;

namespace OverviewServer.Utilities.Callbacks.User
{
	//EXAMPLE: From libplctag - needs to be called from program.cs
	public class CallbackEx
	{
		public static void Run()
		{
			...
			var myUserData = new TestUserData();
		...
		//The callback being ran in the class above
		public static void MyCallback(int tag_id, int event_id, int status, IntPtr userDataPtr)
		{
			...
			var userData = (TestUserData)gcHandle.Target;

			Console.WriteLine($"Tag Id: {tag_id}    Event Id: {event_id}    Status: {status}     User Data: {userData.Code}");
		}
	}
}
